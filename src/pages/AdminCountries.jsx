...
import { getCountries, addCountry, updateCountry, deleteCountry } from '../firebase/firestore';
import { countriesData } from '../data/countriesData';

const empty = { name: '', slug: '', flag: '', image: '', description: '', highlights: [] };
...
export default function AdminCountries() {
  ...
  const openNew = () => { setEditing('new'); setForm({ ...empty, highlights: [] }); };
  ...
  const save = async () => {
    ...
    try {
      if (editing === 'new') {
        ...
        await updateCountry(editing, form);
      }
      await load();
      cancel();
  ...
  const seedFromLocal = async () => {
    if (!window.confirm('Add local seed countries to Firestore?')) return;
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900">Countries</h1>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={cancel}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-extrabold">{editing === 'new' ? 'New Country' : 'Edit Country'}</h2>
              <button onClick={cancel} className="text-slate-400 hover:text-slate-600"><X className="w-5 h-5" /></button>
            </div>
            <div className="space-y-4">
              {['name', 'slug', 'flag', 'image'].map(k => (
                <div key={k}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-slate-100 bg-slate-50">
              ...
              <th className="text-left px-6 py-3 font-bold text-slate-600">Slug</th>
              <th className="text-left px-6 py-3 font-bold text-slate-600">Flag</th>
            ...
            <tbody>
              {items.map(item => (
                <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-slate-500 font-medium">{item.slug}</td>
              ...
              {!items.length && <tr><td colSpan={4} className="px-6 py-12 text-center text-slate-400 font-medium">No countries yet. Click "Add Country" or "Seed Local".</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
